...
app = typer.Typer(name="Ortelana RAG CLI")
console = Console()
...
def handle_reload() -> None:
    """Recarga las variables del archivo .env en memoria."""
    load_dotenv(override=True)
    console.print("[bold green]✓ Variables de entorno recargadas exitosamente.[/bold green]")
...
def handle_help() -> None:
    ...
    console.print("  [cyan]/reload[/cyan]           - Recarga el archivo .env sin salir")
    ...
    while True:
        try:
            ...
            user_input = console.input(
                f"[bold green]ortelana ({current_p})>[/bold green] "
            ).strip()
            ...
            match cmd:
                ...
                case _ if cmd.startswith("/"):
                    console.print(
                        f"[red]Comando no reconocido '{cmd}'. Usa /help para ver opciones.[/red]\n"
                    )
                case _:
